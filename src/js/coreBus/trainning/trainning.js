    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                app.api({
                    url: '/business/train/search', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            date.push(data[i][0]);
                            newCourse.push(data[i][1]);
                            totalPerson.push(data[i][2]);
                            averagePerson.push(data[i][3]);
                            replyNum.push(data[i][4]);
                        }

                        chartOption.option.series[0].data = newCourse.slice(0).reverse();
                        chartOption.option.series[1].data = totalPerson.slice(0).reverse();
                        chartOption.option.series[2].data = averagePerson.slice(0).reverse();
                        chartOption.option.series[3].data = replyNum.slice(0).reverse();
                        ...
                        try {
                            ...
                            var lineChart = echarts.init(document.getElementById('lineTrainning'));
