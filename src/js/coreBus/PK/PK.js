    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                app.api({
                    url: '/business/pktool/search', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            totalPK.push(data[i][2]);

                        }

                        chartOption.option.series[0].data = newPK.slice(0).reverse();
                        chartOption.option.series[1].data = totalPK.slice(0).reverse();

                        chartOption.option.xAxis.data = date.slice(0).reverse();

                        try {

                            var lineChart = echarts.init(document.getElementById('linePk'));
