    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            settleData: function(resp) { //整理第一个表的数据
                ...
                var dataArray = ['date', 'column0', 'column1'];
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                app.api({
                    url: '/business/projectmanager/search', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        var newPorjectNum = [], //新增项目数量
                            totalPorjectNum = [], //累计项目数量

                            date = [];

                        for (var i = 0; i < data.length; i++) {
                            date.push(data[i][0]);
                            newPorjectNum.push(data[i][1]);
                            totalPorjectNum.push(data[i][2]);
                        }

                        chartOption.option.series[0].data = newPorjectNum.slice(0).reverse();
                        chartOption.option.series[1].data = totalPorjectNum.slice(0).reverse();

                        chartOption.option.xAxis.data = date.slice(0).reverse();

                        try {

                            var lineChart = echarts.init(document.getElementById('lineProject'));
