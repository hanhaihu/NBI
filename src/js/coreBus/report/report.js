    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            settleData: function(resp) { //整理第一个表的数据
                ...
                var dataArray = ['date', 'column0', 'column1', 'column2'];
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                app.api({
                    url: '/business/order/search', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        var newAddReportNum = [], //新增战报数量
                            totalReportNum = [], //累计战报数量
                            rewardNum = [], //赏数量
                            date = [];
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            newAddReportNum.push(data[i][1]);

                            totalReportNum.push(data[i][2]);

                            rewardNum.push(data[i][3]);
                        }

                        chartOption.option.series[0].data = newAddReportNum.slice(0).reverse();
                        chartOption.option.series[1].data = totalReportNum.slice(0).reverse();
                        chartOption.option.series[2].data = rewardNum.slice(0).reverse();
                        ...
                        try {
                            ...
                            var lineChart = echarts.init(document.getElementById('lineReport'));
