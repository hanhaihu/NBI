    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            settleData: function(resp) { //整理第一个表的数据
                ...
                var dataArray = ['date', 'column0', 'column1', 'column2', 'column3', 'column4'];
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                app.api({
                    url: '/business/pay/search', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        var sendNum = [], //发送人数
                            cheekNum = [], //查看人数
                            salayImportNum = [], //工资条导入总次数
                            salaySuccNum = [], //工资条导入成功次数
                            importSucc = [], //导入成功率
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            sendNum.push(data[i][1]);
                            cheekNum.push(data[i][2]);
                            salayImportNum.push(data[i][3]);
                            salaySuccNum.push(data[i][4]);
                            importSucc.push((parseFloat(data[i][5])).toFixed(2));
                        }

                        chartOption.option.series[0].data = sendNum.slice(0).reverse();
                        chartOption.option.series[1].data = cheekNum.slice(0).reverse();
                        chartOption.option.series[2].data = salayImportNum.slice(0).reverse();
                        chartOption.option.series[3].data = salaySuccNum.slice(0).reverse();
                        chartOption.option.series[4].data = importSucc.slice(0).reverse();
                        ...
                        try {
                            ...
                            var lineChart = echarts.init(document.getElementById('lineSalary'));
