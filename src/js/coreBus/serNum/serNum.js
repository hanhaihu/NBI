    ...
    exports.init = function() {
        ...
        Session.prototype = {
            ...
            initSessionType: function() {
                ...
                $('.sessiontype button').on('click', function() {
                    ...
                    switch ($('.sessiontype .active').attr('data-sessiontype')) {
                        ...
                        case "0":
                            titleForBar = "新增服务号总数对比";
                            titleForPillar = "新增服务号分时对比";
                            break;

                        case "1":
                            titleForBar = "活跃服务号总数对比";
                            titleForPillar = "活跃服务号分时对比";

                            break;
                        case "2":
                            titleForBar = "累计服务号总数对比";
                            titleForPillar = "累计服务号分时对比";
                            break;
                ...
                app.api({
                    ...
                    success: function(object) {
                        ...
                        for (var i = 0; i < object.value.model[10].length; i++) {
                            ...
                            TypesSerNum.push(object.value.model[10][i].value);
            ...
            getTopChart: function() { //顶部图表数据的请求和渲染
                ...
                tpQuery.templateType = ($('#session').val() == "all" ? '' : $('#session').val()); //模板的值

                app.api({
                    url: '/business/serviceaccountnum/indicators', //顶部图表数据的请求
                    type: 'POST',
                    data: tpQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        console.log(chartOption.option.series[3].data);

                        chartOption.option.xAxis.data = date.slice(0).reverse();

                        try {

                            var lineChart = echarts.init(document.getElementById('lineSerNum'));
            ...
            getBottomChart: function(titleForBar, titleForPillar) {
                ...
                console.log(TypesSerNum);
                btQuery.templates = TypesSerNum.join(',');
                app.api({
                    url: '/business/serviceaccountnum/dimensions', //底部图表数据的请求和渲染;
                    type: 'POST',
                    data: btQuery,
                    dataType: 'json',
                    success: function(resp) {
                        ...
                        if (titleForPillar) {
                            ...
                            chartOption.optionPillar.title.text = "新增服务号分时对比";
                        ...
                        try {
                            ...
                            var pillarChart = echarts.init(document.getElementById('pillarSerNum')); //初始化柱状图
                        ...
                        var nameData = ['IT助手', '空白模本', '空白应用模板', '快捷入口模板', '快递帮手', '示例应用模板', '销售知识库', '企业文化墙', 'HR助手', '行政助手', '项目战报', '周五真心话', '员工运动', '其它第三方应用模板类型', '其它'];
                        ...
                        if (titleForBar) {
                            ...
                            chartOption.optionPie.title.text = "新增服务号总数对比";
                        ...
                        var pieChart = echarts.init(document.getElementById('pieSerNum')); //初始化饼图
            ...
            initDownLoad: function() {
                ...
                $('.download:eq(0)').click(function() {
                    ...
                    var url = BI.API_PATH + "/business/serviceaccountnum/indicatorsdown" + "?" + $.param(tpQuery);
                    window.open(url);
                });
                ...
                $('.download:eq(1)').click(function() {
                    ...
                    btQuery.indicator = parseInt($('.sessiontype .active').attr('data-sessiontype'));
                    btQuery.templates = TypesSerNum.join(',');
                    var url = BI.API_PATH + "/business/serviceaccountnum/dimensionsdown" + "?" + $.param(btQuery);
                    window.open(url);
                });
            }
        }
